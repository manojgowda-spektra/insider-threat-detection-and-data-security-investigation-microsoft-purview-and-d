...
# Always C:. D: on an Azure VM is the ephemeral temp disk, and the Challenge 3 hunting query
# filters on C:\ExfilStaging - branching to D: makes that query return no rows.
$exfilPath = 'C:\ExfilStaging'
if (-not (Test-Path $exfilPath)) { New-Item -Path $exfilPath -ItemType Directory -Force | Out-Null }
Set-Content -Path (Join-Path $exfilPath 'README-ZavaEvidence.txt') -Value "Use this folder as the local exfiltration-like staging location during Challenge 2." -Encoding UTF8 -Force
Write-Host "Evidence staging folder: $exfilPath" -ForegroundColor Green
# Report Python readiness. The Custom Script Extension installs Python and the packages, but the
# Challenge 1 validation grades them, so surface the state here instead of failing later.
$pythonCmd = $null
foreach ($c in @('python.exe','py.exe')) { $g = Get-Command $c -ErrorAction SilentlyContinue; if ($g) { $pythonCmd = $g.Source; break } }
if ($pythonCmd) {
    $pyVer = & $pythonCmd --version 2>&1
    Write-Host "Python detected: $pyVer" -ForegroundColor Green
    foreach ($m in @('requests','msal')) {
        & $pythonCmd -c "import $m" 2>$null
        if ($LASTEXITCODE -eq 0) { Write-Host "Python package '$m' is importable." -ForegroundColor Green }
        else { Write-Warning "Python package '$m' is NOT importable. Run: $pythonCmd -m pip install $m" }
    }
} else {
    Write-Warning 'Python was not found on PATH. Challenge 5 and the Challenge 1 validation both need it.'
}
Write-Step 'Preparation complete. Review C:\LabFiles\ZavaHRData.csv before importing it into the Purview HR connector.'
'@
    }

    function Enable-DefenderAntivirus {
        # The lab grades an alert that only appears if Defender Antivirus real-time protection is
        # running. Azure Windows Server images frequently ship with it disabled or passive, so make
        # the state explicit here rather than letting Challenge 2 fail with no explanation.
        try {
            if (-not (Get-Command Set-MpPreference -ErrorAction SilentlyContinue)) {
                Write-Log 'Defender Antivirus cmdlets are unavailable. Installing the Windows Defender feature.'
                try { Install-WindowsFeature -Name Windows-Defender -ErrorAction Stop | Out-Null } catch { Write-Log "Windows-Defender feature install failed. $_" }
            }
            Set-MpPreference -DisableRealtimeMonitoring $false -ErrorAction Stop
            Set-MpPreference -MAPSReporting Advanced -SubmitSamplesConsent SendSafeSamples -ErrorAction SilentlyContinue
            try { Update-MpSignature -ErrorAction Stop } catch { Write-Log "Signature update failed - detection may lag. $_" }
            $mp = Get-MpComputerStatus -ErrorAction Stop
            Write-Log "Defender Antivirus: RealTimeProtection=$($mp.RealTimeProtectionEnabled) AMService=$($mp.AMServiceEnabled) Signature=$($mp.AntivirusSignatureVersion)"
            if (-not $mp.RealTimeProtectionEnabled) { Write-Log 'WARNING: real-time protection is still off. The Challenge 2 EICAR detection will not raise an alert.' }
        }
        catch { Write-Log "Could not confirm Defender Antivirus state. The Challenge 2 EICAR alert may not fire. $_" }
    }
...
# The EICAR signature is assembled at runtime from fragments. Held as one literal it would be
# detected inside this script file itself, so Defender would quarantine the script before the
# learner could ever run it. Only the file it writes should trip detection.
$eicarParts = @(
    'X5O!P%@AP[4\PZX54(P^)7CC)7}',
    '$EICAR-STANDARD-ANTIVIRUS-',
    'TEST-FILE!$H+H*'
)
$eicar = -join $eicarParts
...
config.json is missing or authentication fails, complete the Challenge 5 manual fallback,
...
CHALLENGE3_NOTES_PATH = LAB_ROOT / "Challenge3-HuntingNotes.txt"
...
def write_outputs(
    ...
    payload = {
        ...
        "challenge3HuntingNotes": challenge3_notes,
    ...
    lines.extend([
        "Challenge 3 hunting notes:",
        f"- Path: {challenge3_notes.get('path')}",
        f"- Exists: {challenge3_notes.get('exists')}",
        f"- Length: {challenge3_notes.get('length', 0)}",
    ])
    if challenge3_notes.get("content"):
        lines.extend(["- Preview:", challenge3_notes["content"][:3000], ""])
    lines.extend([
...
def main() -> int:
    ...
    parser.add_argument("--challenge3-notes", default=str(CHALLENGE3_NOTES_PATH), help="Challenge 3 hunting notes to incorporate into final report")
    ...
    challenge3_notes = read_text_if_exists(Path(args.challenge3_notes))
    dsi_case_json = load_json_if_exists(Path(args.dsi_case_json))
    write_outputs(recent_alerts, lab_matches, dsi_inventory, challenge3_notes, dsi_case_json, Path(args.json_output), Path(args.report_output), cutoff, args.vm_name)
...
'@
    ...
    function Write-GraphBootstrapStatus {
        ...
        try {
            $statusObject = [ordered]@{
                ...
                expectation = 'Best-effort Graph app bootstrap can fail when password grant is blocked, MFA/Conditional Access is enforced, or delegated directory writes/admin consent are unavailable. If config.json is missing or authentication fails, use the manual Challenge 5 app-registration fallback.'
            }
    ...
    function Try-CreateGraphAlertExportApp {
        ...
        if (Test-Path $configPath) { Write-GraphBootstrapStatus -Status 'skipped-existing-config' -Message 'config.json already existed. Use the Challenge 5 smoke test to verify authentication; if it fails, use the manual app-registration fallback.' -ConfigPath $configPath; return }
        if (-not (Test-IsRealBootstrapValue $AzureTenantID)) {
            $message = 'AzureTenantID did not contain a real tenant value. Bootstrap will not write placeholder Graph credentials. Use the manual Challenge 5 fallback to create config.json.'
        ...
        if ([string]::IsNullOrWhiteSpace($token)) {
            $message = 'Best-effort app bootstrap could not obtain a delegated Graph token. Use the manual Challenge 5 fallback to create config.json.'
            Write-GraphBootstrapStatus -Status 'manual-fallback-required' -Message $message -ConfigPath $configPath
            return
        }
        try {
            ...
            $config = [ordered]@{
                ...
                note = 'Graph client values are VM-local only. They are not ARM outputs. If token acquisition or Graph calls fail, complete the manual Entra app registration fallback in Challenge 5 and update this file.'
            }
            ($config | ConvertTo-Json -Depth 10) | Set-Content -Path $configPath -Encoding UTF8 -Force
            Write-GraphBootstrapStatus -Status 'best-effort-created' -Message 'config.json was created by bootstrap. Run the Challenge 5 smoke test and use the manual fallback if authentication or Graph access fails.' -ConfigPath $configPath -TenantId $AzureTenantID -ClientId $app.appId -ClientSecret $secret
        }
        catch {
            $message = "Best-effort Graph app/config creation failed. Use the manual Challenge 5 fallback to create or repair config.json. $_"
            Write-Log $message
            Write-GraphBootstrapStatus -Status 'manual-fallback-required' -Message $message -ConfigPath $configPath
        }
    }
...
- This bootstrap does not onboard the VM to Defender for Endpoint. Download the tenant-specific onboarding package from your Defender portal during Challenge 2.
...
- If C:\LabFiles\config.json is missing or authentication fails, use the manual fallback in Challenge 5.
"@
    }
