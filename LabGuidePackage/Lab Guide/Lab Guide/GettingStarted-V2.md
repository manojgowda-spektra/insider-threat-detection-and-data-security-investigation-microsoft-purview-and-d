...
This lab is written for a fresh Microsoft 365 sandbox tenant. You should assume that the tenant starts with no useful incidents, alerts, device telemetry, audit history, sensitivity labels, app registrations, or named test users unless the deployment creates them locally on the lab VM or you create them during the exercises.
...
> [!Important]
> The same lab admin account, <inject key="AzureAdUserEmail"></inject>, represents the departing Zava design engineer in the exercises. The deployment does not create separate Microsoft 365 user accounts.
...
The exercises are intentionally built around evidence that you create during the session. Do not expect historical alerts or incidents to exist in the tenant before you begin.

## Validation checkpoints

The lab guide contains one CloudLabs validation checkpoint in each exercise. The five canonical validation steps are:
...
- **Microsoft Graph security alerts v2** is used in the final exercise to export Defender for Endpoint alert evidence.
...
- `C:\LabFiles\DSIExports\`, the folder where you save DSI or eDiscovery exported evidence in Exercise 4.
...
This automation is best effort. If `C:\LabFiles\config.json` is missing or authentication fails, you will create the app registration manually in Exercise 5.
...
- **Sensitivity label exploration** in the investigation exercise is no-results-safe. The lab does not depend on newly published labels or a required labeled-content count.
