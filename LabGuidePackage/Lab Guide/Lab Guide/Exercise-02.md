...
In this task, you will connect to the lab VM, sign in to Microsoft 365 portals with the CloudLabs-provided account, and confirm the VM and account values you will use throughout the exercise.
...
8. If any of the expected paths return **False**, return to Exercise 1 and rerun the lab preparation helper before continuing.
...
In this task, you will copy staged design files to an exfiltration-like staging folder. This creates endpoint file activity that you will hunt in Exercise 3.

1. In the elevated PowerShell window, choose the evidence staging path. The helper uses a secondary volume when available; otherwise it uses **C:\ExfilStaging**.
...
3. Create a small activity log that you can reference during the later hunting and case-report exercises.
...
4. Record the exact destination path shown by PowerShell. You will use it in Exercise 3 when querying **DeviceFileEvents**.

> [!Tip]
> A secondary or removable drive is preferred when available because it looks more like exfiltration staging. If your VM only has the system drive, **C:\ExfilStaging** is acceptable for this lab. The hunting queries in the next exercise will use the path you actually created.
...
> [!Note]
> Email telemetry may not appear in Microsoft Defender XDR advanced hunting at the same speed as endpoint telemetry. In Exercise 3, you will use retry-oriented queries and record whether email rows are available in your tenant during the lab session.
...
> [!Important]
> The EICAR test is the real Defender for Endpoint alert source used later in this lab. You are not expected to receive an Insider Risk Management alert or a correlated Insider Risk Management incident during this exercise.
...
> [!Note]
> Microsoft Learn documents this as a single setting under **Insider Risk Management settings** > **Data sharing** > **Sharing data with other Microsoft security solutions**. When the setting is enabled, Insider Risk Management alert severity levels can be shared with Microsoft Defender XDR, DLP alerts, and Communication Compliance. It does not force the policy from Exercise 1 to score an Insider Risk Management alert during the lab session.

> [!Important]
> This Purview sharing step is manual and non-graded in CloudLabs. The automated validation for this exercise checks real Defender for Endpoint onboarding and EICAR evidence; it does not grade your manual Purview data-sharing checklist note.
