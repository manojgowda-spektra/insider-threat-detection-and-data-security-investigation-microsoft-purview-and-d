# Challenge 05: Python and Microsoft Graph alert export

### Estimated Duration: 45 Minutes

## Scenario

Zava Manufacturing's investigation team needs a portable evidence package that includes Defender for Endpoint alert data, user evidence, and exported DSI/eDiscovery evidence from the hunting and data security investigation work you completed earlier. In this challenge, you will use the prepared Python starter file on the lab VM to authenticate to Microsoft Graph, query the Microsoft Graph security `alerts_v2` endpoint, filter for recent Microsoft Defender for Endpoint evidence from this lab, and export both a JSON evidence file and a plain-text case report.
...
5. Confirm that the lab files and starter script exist.
   ...
   Test-Path C:\LabFiles\Challenge3-HuntingNotes.txt
...
7. Confirm the Python packages used by this challenge are installed.
...
2. Read the script end to end before changing anything. It is already near-complete; the areas you may need to adjust are the tenant and client configuration it reads from `config.json` and the severity filter. Do not replace the whole file unless your facilitator specifically instructs you to.

   ```powershell
   Select-String -Path C:\LabFiles\get_insider_alerts.py -Pattern 'alerts_v2','SecurityAlert.Read.All','@odata.nextLink','createdDateTime','microsoftDefenderForEndpoint','.default'
   ```
...
7. Confirm the output and evidence-import paths are present.
   ...
   ```powershell
   Select-String -Path C:\LabFiles\get_insider_alerts.py -Pattern 'insider_risk_case.json','insider_risk_case_report.txt','Challenge3-HuntingNotes.txt','DSIExports','dsi_case_evidence.json','labScopedMdeAlerts','labScopedMdeAlertSummaries','dsiCaseEvidenceParsed','dsiExportInventory'
   ```

8. Save the file if you made any adjustments.
...
In this task, you will query recent alerts, export evidence, and review the generated case files. The script imports your earlier hunting notes and the real DSI/eDiscovery export metadata from Challenge 4; it does not ask you to type a new ungraded narrative summary in this challenge.

1. Confirm the earlier notes and required DSI/eDiscovery export index exist. If the DSI export index is missing, return to Challenge 4 and complete the `C:\LabFiles\DSIExports` export collection before running the Graph export.
...
5. If all runs still return zero alerts, wait five to ten minutes for Defender alert ingestion and rerun the script. Confirm that Challenge 2 successfully onboarded the VM and triggered the EICAR test alert in the Microsoft Defender portal. Do not widen the lookback window unless you are intentionally troubleshooting; the default 72-hour cutoff is designed to avoid stale or unrelated alerts.
...
7. Confirm that the JSON export includes the server-side Graph cutoff metadata, the lab VM name used for filtering, imported hunting notes, normalized DSI/eDiscovery index, and recursive DSI/eDiscovery file inventory. The JSON shape must include top-level `labScopedMdeAlerts` and `labScopedMdeAlertSummaries` arrays plus `challenge3HuntingNotes`, `dsiCaseEvidenceParsed`, and `dsiExportInventory` objects.
   ...
   $case.challenge3HuntingNotes | Select-Object path, exists, length, truncated
...
8. Confirm that the text report exists and contains alert identifiers, hunting findings, DSI/eDiscovery export metadata, the recursive export file inventory, and the Graph cutoff scope.
   ...
   Select-String -Path C:\LabFiles\insider_risk_case_report.txt -Pattern 'Server-side Graph cutoff','Lab VM filter','Challenge 3 hunting notes','DSI/eDiscovery normalized case evidence','DSI/eDiscovery export inventory','Recent lab-scoped MDE alerts matched','Alert ID'
   ```
...
13. In the `dsiCaseEvidenceParsed` section, review the normalized DSI/eDiscovery evidence index exported in Challenge 4 and confirm that it ties back to **Insider Threat Case - Design Engineer**. Then review `dsiExportInventory.files` to confirm the real portal export files and any subfolders under `C:\LabFiles\DSIExports` were inventoried recursively.
...
15. Confirm that the report includes the content of `C:\LabFiles\Challenge3-HuntingNotes.txt`, parsed metadata from the normalized index at `C:\LabFiles\DSIExports\dsi_case_evidence.json`, a recursive file inventory for `C:\LabFiles\DSIExports`, and the server-side Graph cutoff used for the export.
...
3. Run the validation check for this challenge.
