...
• Graph client values are never emitted as ARM placeholders or ARM outputs. Real Microsoft Graph client ID and secret values are written only to VM-local files such as C:\LabFiles\config.json and C:\LabFiles\GraphBootstrapStatus.json when best-effort bootstrap or the Exercise 5 manual setup succeeds.
