...
# labScopedMdeAlertSummaries, challenge3HuntingNotes, dsiCaseEvidenceParsed, and dsiExportInventory.
...
try {
    ...
    # Fetch the tenant's actual recent alerts so the learner's export can be cross-checked
    # against them. The previous code requested $top=1 and only asserted that the response had a
    # 'value' property, so an empty result array passed and the returned alerts were never
    # compared to anything - it proved only that a token could be obtained.
    $alertsUri = 'https://graph.microsoft.com/v1.0/security/alerts_v2?$filter=' + $encodedFilter + '&$top=100'
    $graphAlerts = New-Object System.Collections.Generic.List[object]
    $nextUri = $alertsUri
    $page = 0
    while (-not [string]::IsNullOrWhiteSpace($nextUri) -and $page -lt 5) {
        $page++
        try { $graphResponse = Invoke-RestMethod -Method Get -Uri $nextUri -Headers $graphHeaders -ErrorAction Stop }
        catch { Emit-Result -Status 'Failed' -Message "Microsoft Graph /security/alerts_v2 call failed: $($_.Exception.Message)" -Details @{ TenantId = $tenantId; ClientId = $clientId; Uri = $nextUri }; return }
        if ($null -eq $graphResponse -or $null -eq $graphResponse.PSObject.Properties['value']) {
            Emit-Result -Status 'Failed' -Message 'Microsoft Graph security alerts API did not return a collection response with a value property.' -Details @{ Uri = $nextUri }
            return
        }
        foreach ($a in @($graphResponse.value)) { $graphAlerts.Add($a) | Out-Null }
        $nextUri = $graphResponse.'@odata.nextLink'
    }
    # Fail closed: no alerts in the tenant means the learner's export cannot be corroborated.
    if ($graphAlerts.Count -lt 1) {
        Emit-Result -Status 'Failed' -Message "Microsoft Graph returned no alerts created since $graphCutoffText, so the exported alert evidence cannot be corroborated. Run the EICAR test in Challenge 2, wait for the alert to surface, then re-run the export." -Details @{ Uri = $alertsUri; CutoffUtc = $graphCutoffText }
        return
    }
    # Real alert ids straight from Graph. providerAlertId is the MDE-side identifier and is not
    # addressable at /security/alerts_v2/{id}, so it is kept only as a match target.
    $graphAlertIdSet = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::OrdinalIgnoreCase)
    foreach ($a in $graphAlerts) {
        foreach ($idProp in @('id','providerAlertId')) {
            $v = [string]$a.$idProp
            if (-not [string]::IsNullOrWhiteSpace($v)) { [void]$graphAlertIdSet.Add($v) }
        }
    }
    ...
    foreach ($requiredField in @('generatedUtc','cutoffUtc','labVmName','labScopedMdeAlertCount','labScopedMdeAlertSummaries','labScopedMdeAlerts','challenge3HuntingNotes','dsiCaseEvidenceParsed','dsiExportInventory')) {
    ...
    # The decisive check. Every id above came out of a file the learner can hand-write, so at
    # least one of them must also appear in the alerts Graph actually returned. Without this the
    # validator passes on a fabricated JSON file.
    $corroborated = @($alertIds | Where-Object { $graphAlertIdSet.Contains([string]$_) })
    if ($corroborated.Count -lt 1) {
        Emit-Result -Status 'Failed' -Message "None of the alert identifiers in insider_risk_case.json match an alert that Microsoft Graph actually returned for this tenant. The export must be generated by running get_insider_alerts.py against live data, not written by hand." -Details @{ Path = $exportPath; ExportedIdCount = $alertIds.Count; GraphAlertCount = $graphAlerts.Count }
        return
    }

    $challenge3 = $exportParsed.challenge3HuntingNotes
    if ($null -eq $challenge3 -or $challenge3.exists -ne $true -or [int]($challenge3.length) -lt 20 -or [string]::IsNullOrWhiteSpace([string]$challenge3.content)) {
        Emit-Result -Status 'Failed' -Message 'The JSON export must include non-empty challenge3HuntingNotes as produced by the delivered script from the Challenge 3 hunting notes file.' -Details @{ Path = $exportPath; Challenge3Exists = $challenge3.exists; Challenge3Length = $challenge3.length }
        return
    }
    ...
    foreach ($reportSection in @('Challenge 3 hunting notes','DSI/eDiscovery normalized case evidence','DSI/eDiscovery export inventory','Matched alerts')) {
